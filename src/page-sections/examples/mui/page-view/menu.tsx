...
export default function MuiMenuPageView() {
  ...
  // icon menu list
  const [iconMenuEl, setIconMenuEl] = useState<null | HTMLElement>(null)

  const handleIconMenuClose = () => setIconMenuEl(null)
  const handleIconMenuClick = (event: MouseEvent<HTMLButtonElement>) =>
    setIconMenuEl(event.currentTarget)

  // selected menu
  const [selectedIndex, setSelectedIndex] = useState(1)
  const [selectedMenuEl, setSelectedMenuEl] = useState<null | HTMLElement>(null)
  ...
  // max height menu
  const [maxMenuEl, setMaxMenuEl] = useState<null | HTMLElement>(null)

  const handleMaxMenuClose = () => setMaxMenuEl(null)
  const handleMaxMenuClick = (event: MouseEvent<HTMLElement>) => {
    setMaxMenuEl(event.currentTarget)
  }

  return (
    <ComponentPageLayout title="Menu">
      <Grid container spacing={3}>
        ...
        <Grid item md={4} sm={6} xs={12}>
          <Block title="Selected Menu">
            ...
            <Menu
              id="lock-menu"
              anchorEl={selectedMenuEl}
              open={Boolean(selectedMenuEl)}
        ...
        <Grid item md={4} sm={6} xs={12}>
          <Block title="Max Height">
            ...
            <Menu anchorEl={maxMenuEl} open={Boolean(maxMenuEl)} onClose={handleMaxMenuClose}>
              <Scrollbar sx={{ maxHeight: 200, width: '20ch' }}>
                {options2.map((option) => (
                  <MenuItem key={option} selected={option === 'Pyxis'} onClick={handleMaxMenuClose}>
                    {option}
                  </MenuItem>
                ))}
              </Scrollbar>
