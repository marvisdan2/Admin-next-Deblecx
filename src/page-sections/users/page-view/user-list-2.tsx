...
const BodyTableRow = styled(TableRow, {
  ...
  cursor: 'pointer',
  ...(active && { backgroundColor: theme.palette.grey[isDark(theme) ? 700 : 100] }),
}))
...
export default function UserList2PageView() {
  ...
  const [selectedUser, setSelectedUser] = useState<(typeof users)[0]>()
  ...
  } = useMuiTable({ defaultOrderBy: 'name', defaultRowsPerPage: 10 })

  let filteredUsers = stableSort(users, getComparator(order, orderBy)).filter((item) => {
    if (searchFilter) return item.name.toLowerCase().includes(searchFilter.toLowerCase())
    else return true
  })
