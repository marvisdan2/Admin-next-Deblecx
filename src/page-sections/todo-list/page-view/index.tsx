...
type KEYS = keyof typeof VIEW_COLUMNS
...
export default function TodoListPageView() {
  ...
  const onDragEnd = (
    result: DropResult,
    columns: typeof boardList,
    setColumns: typeof setBoardList
  ) => {
    if (!result.destination) return
    const { source, destination } = result

    if (source.droppableId !== destination.droppableId) {
      const sourceColumn = columns[source.droppableId as KEYS]
      const destColumn = columns[destination.droppableId as KEYS]
      ...
      const column = columns[source.droppableId as KEYS]
      ...
      <Grid container spacing={3}>
        <DragDropContext onDragEnd={(result) => onDragEnd(result, boardList, setBoardList)}>
          {Object.entries(boardList).map(([columnId, column]) => {
            return (
              <Grid item xs={12} sm={6} lg={4} key={columnId}>
                <Card sx={{ height: '100%', maxHeight: 750 }}>
                  {columnId === 'todo' ? (
                    <TodoForm
                      title={column.name}
                      show={showAddTodoForm}
                      handleOpen={() => setShowAddTodoForm(true)}
