...
import {
  ...
  ProductFilterKeys,
  ProductFilterValues,
} from './product-filter-sidebar/type'
...
export default function Section2() {
  ...
  const [filters, setIsFilters] = useState<ProductFilters>(defaultFilters)

  const handleChangeFilters = (key: ProductFilterKeys, values: ProductFilterValues) => {
    setIsFilters((prev) => ({ ...prev, [key]: values }))
  }
  ...
  return (
    <div className="py-10">
      <FilterActions
        sortBy={sortBy}
        handleSidebar={() => setIsOpenFilter(true)}
      ...
      <ProductFilterSidebar
        filters={filters}
        open={isOpenFilter}
        filterApplied={hasFilterApplied}
