...
export default function ProductGridPageView() {
  ...
  const handleChangeTab = (_: SyntheticEvent, newTab: string) => setSelectTab(newTab);

  const FILTER_PRODUCTS = products
    .slice(0, pageSize * pageIndex)
    .filter((item) => {
    ...
    <div className="pt-2 pb-4">
      <TabContext value={selectTab}>
        ...
        <Grid container spacing={3}>
          ...
          {/* LOAD MORE BUTTON */}
          {FILTER_PRODUCTS.length !== 0 && products.length > FILTER_PRODUCTS.length ? (
            <Grid item xs={12}>
              <FlexRowAlign mt={2}>
