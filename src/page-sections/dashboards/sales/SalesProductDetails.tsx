...
export default function SalesProductDetails() {
  ...
  const chartOptions: ApexOptions = {
    ...
    chart: { background: "none", fontFamily: theme.typography.fontFamily },
    plotOptions: {
      radialBar: {
        hollow: { size: "20%" },
        dataLabels: { name: { show: false }, value: { show: false } },
        track: { margin: 15, background: theme.palette.divider }
      }
    },
  ...
  return (
    <Card className="p-3">
      ...
      <Chart options={chartOptions} series={[75, 50, 25]} type="radialBar" height={330} />
    </Card>
  );
}
