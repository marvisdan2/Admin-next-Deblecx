...
const ListItem = styled('li')(({ theme }) => ({
  paddingBottom: 8,
  color: theme.palette.text.secondary,
  '::marker': { color: theme.palette.text.secondary, fontSize: '140%' },
}))

const ListGroup = styled('div')(({ theme }) => ({
  gap: 16,
  display: 'grid',
...
export default function Overview() {
  return (
    <Grid container spacing={3}>
      {OVERVIEW_DATA.map(({ id, title, items }) => (
      ...
      <Grid item xs={12}>
        <Card sx={{ position: 'relative', minHeight: 300, p: 3 }}>
