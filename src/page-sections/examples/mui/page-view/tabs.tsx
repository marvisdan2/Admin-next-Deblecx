import { useState, SyntheticEvent, PropsWithChildren } from 'react'
// MUI ICON COMPONENTS
...
function TabPanel({ children, value, index, ...props }: TabPanelProps) {
  return (
    <Box mt={2} role="tabpanel" hidden={value !== index} {...props}>
      {value === index && (
        <Box
          p={3}
          boxShadow={1}
          borderRadius={2}
          bgcolor={({ palette: { mode } }) => (mode === 'dark' ? 'grey.700' : 'white')}
        >
...
export default function MuiTabsPageView() {
  ...
  return (
    <ComponentPageLayout title="Tabs">
