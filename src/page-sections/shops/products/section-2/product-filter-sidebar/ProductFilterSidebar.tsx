...
// TYPED
import { ProductFilterKeys, ProductFilterValues, ProductFilters } from './type'
...
interface Props {
  ...
  onChangeFilters: (keys: ProductFilterKeys, values: ProductFilterValues) => void
}
// ==============================================================
...
export default function ProductFilterSidebar({
  ...
  const handleChangePriceRange = useCallback(
    (newValue: number[]) => onChangeFilters('price', newValue),
    [onChangeFilters]
  )

  // FOR RATING
  const handleChangeRating = useCallback(
    (newValue: number) => onChangeFilters('rating', newValue),
    [onChangeFilters]
  )
  ...
  const handleChangeColor = useCallback(
    (value: string) => {
      const newValue = filters.color.includes(value)
  ...
  const handleChangeCategory = useCallback(
    (value: string) => {
      const newValue = filters.category.includes(value)
  ...
  const handleChangeGender = useCallback(
    (value: string) => {
      const newValue = filters.gender.includes(value)
        ? filters.gender.filter((gender) => gender !== value)
            ...
            <div>
              ...
              <Stack direction="row" alignItems="center" spacing={2} mb={2}>
                <TextField
                  ...
                  onChange={(e) => handleChangePriceRange([+e.target.value, filters.price[1]])}
                />
                ...
                <TextField
                  ...
                  onChange={(e) => handleChangePriceRange([filters.price[0], +e.target.value])}
                />
              </Stack>
              ...
              <Slider
                ...
                onChange={(_, v) => handleChangePriceRange(v as number[])}
                marks={[
                  { value: 0, label: '$0' },
                  { value: 50, label: '$50' },
                  { value: 100, label: '$100' },
