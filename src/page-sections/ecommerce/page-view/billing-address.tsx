...
export default function BillingAddressPageView() {
  ...
  const [selectBillingAddress, setSelectBillingAddress] = useState(1)

  // MODAL CLOSE HANDLER
  const handleClose = () => setOpenModal(false)

  const handleSelectBillingAddress = (id: number) => {
        ...
        <Grid item md={8} xs={12}>
          ...
          <Stack gap={2}>
            {[1, 2, 3].map((item) => (
              <BillingAddressCard
                key={item}
                selected={selectBillingAddress === item}
                handleChange={() => handleSelectBillingAddress(item)}
              />
            ))}
          </Stack>
