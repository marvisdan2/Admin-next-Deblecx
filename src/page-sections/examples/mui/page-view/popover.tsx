...
export default function MuiPopoverPageView() {
  const [basicEl, setBasicEl] = useState<HTMLButtonElement | null>(null)
  const handleBasicClose = () => setBasicEl(null)
  const handleBasicClick = (event: MouseEvent<HTMLButtonElement>) => {
  ...
  const handleHover = (e: MouseEvent<HTMLElement>) => setHoverEl(e.currentTarget)

  const [transitionEl, setTransitionEl] = useState<HTMLButtonElement | null>(null)
  const [trans, setTrans] = useState('z')
  const handleTransClose = () => setTransitionEl(null)
  const handleTransClick = (val: string) => (event: MouseEvent<HTMLButtonElement>) => {
    setTransitionEl(event.currentTarget)
    setTrans(val)
  }

  return (
    <ComponentPageLayout title="Popover">
        ...
        <Grid item lg={6} xs={12}>
          <Block title="Transition">
            <Stack justifyContent="center" alignItems="center">
              ...
              <Popover
                anchorEl={transitionEl}
                open={Boolean(transitionEl)}
