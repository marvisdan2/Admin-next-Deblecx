...
const StyledRoot = styled(Card)(({ theme }) => ({
  ...
  '& .item-content': { flexGrow: 1 },
  '& .download-btn': { color: theme.palette.primary.main },
  '& .cancel-btn': { color: theme.palette.error.main },
}))
  ...
  return (
    <StyledRoot>
      ...
      {downloadList.map((item) => (
        <FlexBox alignItems="center" mt={2} gap={1} key={item.id}>
          <Box width={40}>
          ...
          <div className="item-content">
            <FlexBetween>
              ...
              {item.isDownloading ? (
                <ButtonBase disableRipple disableTouchRipple className="cancel-btn">
                  Cancel
                </ButtonBase>
              ) : null}
            </FlexBetween>
