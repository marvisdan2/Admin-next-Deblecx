...
export default function Layout({ children, showTopActions = true }: LayoutProps) {
  const { palette, direction } = useTheme()
  const [openSidebar, setOpenSidebar] = useState(false)

  const upSm = useMediaQuery((theme: Theme) => theme.breakpoints.up('sm'))
  const downMd = useMediaQuery((theme: Theme) => theme.breakpoints.down('md'))
  const downLg = useMediaQuery((theme: Theme) => theme.breakpoints.down('xl'))
  ...
  return (
    <Box py={3} height="100%">
      <Card sx={{ display: 'flex', minHeight: 800 }}>
        {/* SIDEBAR */}
        ...
        <Box flexGrow={1}>
          {/* TOP ACTIONS */}
          {showTopActions && (
            <FlexBetween p={3} flexWrap="wrap" borderBottom={`1px solid ${palette.divider}`}>
              <FlexBox gap={1} alignItems="center">
                {/* SHOW MENU BUTTON WHEN DEVICE IS MEDIUM */}
                {downMd && (
                  <IconButton color="secondary" onClick={() => setOpenSidebar(true)}>
                    <Menu fontSize="small" />
                  </IconButton>
                )}
                ...
                >
                  ...
                  <Tooltip title="Previous Page">
                    <IconButton color="inherit" size="small">
                      <ArrowBackIos
                        fontSize="inherit"
                        sx={{ rotate: direction === 'rtl' ? '180deg' : 0 }}
                      />
                    </IconButton>
                  </Tooltip>
