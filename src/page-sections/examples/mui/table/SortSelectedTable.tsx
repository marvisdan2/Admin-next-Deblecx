...
interface CustomTableHeadProps {
  ...
  rowCount: number
  numSelected: number
  onSelectAllClick: (data: string[]) => (e: ChangeEvent<HTMLInputElement>) => void
  onRequestSort: (e: MouseEvent<unknown>, property: keyof Data) => void
}
...
const createData = (
  ...
  carbs: number,
  protein: number
): Data => ({ name, calories, fat, carbs, protein })
      ...
      <TableRow>
        <TableCell padding="checkbox">
          <Checkbox
            size="small"
            color="primary"
            onChange={onSelectAllClick(rows.map((n) => n.name))}
...
function TableToolbar({ numSelected }: { numSelected: number }) {
  return (
    <Toolbar
      sx={{
    ...
    >
      {numSelected > 0 ? (
        ...
        <Paragraph fontWeight={600} flex="1 1 100%" id="tableTitle">
          Nutrition
        </Paragraph>
      )}
...
export default function SortSelectedTable() {
  ...
  return (
    <Paper sx={{ width: '100%', boxShadow: 2, borderRadius: 2, overflow: 'hidden' }}>
        ...
        <Scrollbar>
          <Table sx={{ minWidth: 750 }}>
            ...
            <TableBody>
              {visibleRows.map((row, index) => {
                ...
                return (
                  <TableRow
                    ...
                    onClick={(event) => handleSelectRow(event, row.name as string)}
                    sx={{ cursor: 'pointer' }}
                  >
                    <TableCell padding="checkbox">
              ...
              {emptyRows(page, rowsPerPage, rows.length) > 0 && (
                <TableRow>
                  <TableCell colSpan={6} />
                </TableRow>
              )}
